...
def main():
    ...
    with st.sidebar:
        ...
        selected = option_menu(
            menu_title=None,
            options=["Calculator", "Model Insights", "Clinical Guidelines", "About"],
    ...
    elif selected == "Clinical Guidelines":
...
def clinical_guidelines_page():
    """Page showing clinical guidelines and recommendations"""
    st.markdown('<h1 class="main-header">Clinical Guidelines & Best Practices</h1>', unsafe_allow_html=True)
    ...
    with st.container():
        ...
        st.markdown("""
        This precision medicine approach for post-thyroidectomy levothyroxine dosing integrates 
        machine learning insights with established clinical protocols to optimize patient outcomes.
        The following guidelines are based on the analysis of 619 patients and validated through 
        rigorous cross-validation.
        """)
        
        st.markdown('</div>', unsafe_allow_html=True)
    
    # BMI-Stratified Dosing Protocol
    with st.container():
        st.markdown('<div class="stcard">', unsafe_allow_html=True)
        st.subheader("BMI-Stratified Dosing Protocol")
        
        # Create tabs for different BMI categories
        tab1, tab2, tab3 = st.tabs(["Low BMI (≤27.67 kg/m²)", "Medium BMI (27.67-34.98 kg/m²)", "High BMI (>34.98 kg/m²)"])
        
        with tab1:
            st.markdown("### Low BMI Protocol (≤27.67 kg/m²)")
            st.markdown("**Success Rate:** 57.1%")
            ...
            with col1:
                st.markdown("""
                #### Initial Dosing
                - Start with 100 mcg daily for 7 days
                - Typical range: 100-100 mcg
                - Initial dose stability tends to be high
            ...
            with col2:
                ...
                - Low BMI patients generally achieve euthyroid status more predictably
                - Less frequent dose adjustments typically needed
                - Consider 75 mcg initial dose for elderly (>65 years) or those with cardiovascular disease
                - Monthly monitoring if PTH > 7 pmol/L
                """)
        
        with tab2:
            st.markdown("### Medium BMI Protocol (27.67-34.98 kg/m²)")
            st.markdown("**Success Rate:** 35.7%")
            ...
            with col1:
                ...
                #### Initial Dosing
                - Start with 100 mcg daily for 7 days
                - Typical range: 100-125 mcg
                - Consider more frequent early monitoring
                
                #### Monitoring
                - Check TSH, T4 at 6-8 weeks
                - Consider additional check at 3-4 weeks in high-risk patients
                - Adjust dose if TSH outside 0.3-4.5 mIU/L
                - Consider T3 monitoring in selected cases
                """)
            ...
            with col2:
                st.markdown("""
                #### Special Considerations
                - Medium BMI patients show moderate dose variability
                - More frequent adjustments may be needed at 3-month timepoint
                - Be prepared for dose escalation at 6 months (up to 125-138 mcg)
                - Monthly monitoring if PTH > 7 pmol/L
                - Consider weight management counseling
                """)
        
        with tab3:
            st.markdown("### High BMI Protocol (>34.98 kg/m²)")
            st.markdown("**Success Rate:** 35.7%")
            ...
            with col1:
                ...
                #### Initial Dosing
                - Start with 100 mcg daily for 7 days
                - Typical range: 100-150 mcg
                - Anticipate greater need for dose adjustments
                
                #### Monitoring
                - Check TSH, T4 at 4-6 weeks (earlier than standard)
                - Additional check at 3 months strongly recommended
                - Adjust dose if TSH outside 0.3-4.5 mIU/L
                - Consider more frequent TSH monitoring until stable
                """)
                
            with col2:
                st.markdown("""
                #### Special Considerations
                - High BMI patients show highest dose variability
                - Most likely to require significant dose escalation
                - Be prepared for dose increases at 6 months (up to 150 mcg)
                - Monthly monitoring if PTH > 7 pmol/L
                - Weight management strongly recommended
                - Consider clinical nutritionist referral
                """)
    ...
    with st.container():
        st.markdown('<div class="stcard">', unsafe_allow_html=True)
        st.subheader("Structured Monitoring Protocol")
        ...
        with col1:
            ...
            - **TSH:** Primary indicator for dose adjustment (target: 0.3-4.5 mIU/L)
            - **T4:** Secondary assessment for confirmation
            - **T3:** Consider in cases of persistent symptoms despite normal TSH
            - **PTH:** Enhanced monitoring if > 7 pmol/L
            - **Calcium:** Monitor in patients with parathyroid concerns
            
            #### Timing of Assessments
            - **Initial follow-up:** 6-8 weeks (4-6 weeks for high BMI)
            - **Subsequent follow-up:** 3 months, 6 months, then every 6 months
            - **Monthly monitoring:** For patients with PTH > 7 pmol/L
            - **Additional monitoring:** For persistent symptoms or abnormal values
            """)
            
        with col2:
            st.markdown("""
            #### Dose Adjustment Guidelines
            - **If TSH > 4.5 mIU/L:** Increase by 12.5-25 mcg
            - **If TSH < 0.3 mIU/L:** Decrease by 12.5-25 mcg
            - **If symptoms persist:** Consider T3 assessment
            - **For high BMI patients:** Consider more aggressive initial dosing
            - **For elderly or cardiac patients:** More conservative adjustments (12.5 mcg)
            
            #### Patient Education
            - Take levothyroxine in the morning on empty stomach
            - Wait 30-60 minutes before eating
            - Avoid calcium, iron supplements within 4 hours
            - Report symptoms of hypo/hyperthyroidism promptly
    ...
    with st.container():
        st.markdown('<div class="stcard">', unsafe_allow_html=True)
        st.subheader("Special Clinical Considerations")
        
        # Create expandable sections
        with st.expander("Elderly Patients (>65 years)"):
            st.markdown("""
            - Consider starting at lower dose (75-88 mcg)
            - More frequent monitoring initially (4-6 weeks)
            - Smaller dose adjustments (12.5 mcg)
            - Monitor closely for cardiac symptoms
            - Consider more frequent ECG monitoring
            """)
            
        with st.expander("Patients with Cardiovascular Disease"):
            st.markdown("""
            - Start at lower dose (75 mcg)
            - More gradual dose titration
            - Consider cardiology consultation before significant dose increases
            - Monitor heart rate and symptoms closely
            - Consider baseline and follow-up ECG
            """)
            
        with st.expander("Pregnant or Planning Pregnancy"):
            st.markdown("""
            - Target lower TSH (0.1-2.5 mIU/L)
            - More frequent monitoring (every 4-6 weeks)
            - May require dose increase by 25-30% during pregnancy
            - Return to pre-pregnancy dose post-partum
            - Consider endocrinology referral
            """)
            
        with st.expander("Patients with Malabsorption Issues"):
            st.markdown("""
            - Consider higher initial dose
            - More frequent monitoring
            - Take on empty stomach with water
            - Avoid concurrent medications that may affect absorption
            - Consider checking compliance if consistently hypothyroid
            """)
            
        with st.expander("Patients with Residual Thyroid Tissue/Cancer"):
            st.markdown("""
            - May require suppressive therapy (TSH <0.1 mIU/L)
            - Higher doses typically needed
            - More aggressive titration
            - Coordinate with oncology team
            - Monitor for long-term effects of suppression (bone, cardiac)
            """)
